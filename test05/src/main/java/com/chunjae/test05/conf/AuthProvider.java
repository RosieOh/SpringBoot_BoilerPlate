...
public class AuthProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        String name = (String) authentication.getPrincipal(); // 로그인 창에 입력한 id(name)
        ...
        if (userVo != null && passwordEncoder.matches(password, userVo.getPassword()) && userVo.getAct().equals("JOIN")) { // 일치하는 user 정보가 있는지 확인
            ...
            if(userVo.getLev().equals("ADMIN")){
               ...
               roles.add(new SimpleGrantedAuthority("EMP")); // EMP 권한 부여
            } else {
               roles.add(new SimpleGrantedAuthority("USER")); // USER 권한 부여
            }
